...
for(i1 in 1:(p-1)){
	for(j1 in i1:p){
		if((j1-i1 %% p)==1|(i1-j1 %% p==1)){
			prec.mat[i1,j1]=prec.mat[j1,i1]=0.4
		}
	}
}
dim(prec.mat)
tpre<-1*(round(prec.mat,10)!=0)
tpre<-S<-prec.mat 
set.seed(1)
a<-rmt(n*m.carlo, mean = rep(0, d), S, df=dof, sqrt=NULL)
dim(a)

 pt<-proc.time()

data.Non<-a
for(i1 in 1:d){ ## d m.carlo olmali 

X<-data.Non[(20*i1-19):(20*i1),]

X<-as.matrix(X)
xhuge<-huge(X,method="glasso",cov.output = TRUE,nlambda=10)
hs<-huge.select(xhuge, criterion ="stars")
est.mat<-hs$opt.icov
epre<-1*(round(est.mat,10)!=0)
...
fileName5 <- paste(mt,p, "-dimensional-empricalCopula.txt", sep ="")
 
write(c(i1,TP,FN,FP,TN,spe,fdr,fpr,pre,rec,acc,F,MCC),fileName5,ncol=13,append=T,sep = "\t")

dt<-a
names(dt)<-c("X1","X2","X3","X4","X5","X6","X7","X8","X9","X10",
...
########################################################
#
# Compare the true prec.mat and estimated prec.mat. 
...
fileName5 <- paste(cr,p, "-dimensional-nonlinear.txt", sep ="")
write(c(i1,TP,FN,FP,TN,spe,fdr,fpr,pre,rec,acc,F,MCC),fileName5,ncol=13,append=T,sep = "\t")
